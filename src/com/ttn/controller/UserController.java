...
System.out.println("new json" + json);
    ...
    @ResponseBody
    public Boolean isAuthenticateUser(@RequestBody String json , HttpServletRequest request, HttpServletResponse response) throws InvalidCredentialsException, UserDeactivatedException, EmailNotFoundException {
      ...
      Gson gson = new Gson();
		...
		if(new EmailValidator().validate(credentials.getUsernameOrEmail())){
			System.out.println("Email validated");
			if(userService.checkEmailExists(credentials.getUsernameOrEmail())){
				credentials.setUsernameOrEmail(userService.getUsername(credentials.getUsernameOrEmail()));
			}
			...
			if(userService.isUserDeactivatedFromUsername(credentials.getUsernameOrEmail())){
				throw new UserDeactivatedException();
			}
			
			else if(userService.isAuthenticateUserFromUsername(credentials.getUsernameOrEmail(), credentials.getPassword())){
				HttpSession session = request.getSession(true);
				session.setAttribute("username", credentials.getUsernameOrEmail());
				return true;
...
//      }
    ...
    public User getUser(@RequestBody String json ) throws InvalidCredentialsException {
      ...
      json = URLDecoder.decode(json);
		...
		return userService.getUser(credentials.getUsernameOrEmail());
