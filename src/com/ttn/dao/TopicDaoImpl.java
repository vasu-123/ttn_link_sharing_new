...
public class TopicDaoImpl implements TopicDao {
	...
	public Boolean isTopicAlreadyCreated(Topic topic) {
		...
		Session session = sessionFactory.openSession();
		session.beginTransaction();
		Query query = session.createQuery("select count(*) from Topic where createdBy=? and name=?");
		query.setString(0, topic.getCreatedBy().getUsername());
		query.setString(1, topic.getName());
	...
	public Topic getTopic(Integer id) {
		...
		Session session = sessionFactory.openSession();
		session.beginTransaction();
		System.out.print("Why is id null "+id);
