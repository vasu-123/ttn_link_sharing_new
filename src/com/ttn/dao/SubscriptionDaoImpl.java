...
//				+ "subscription on topic.id = subscription.topic_id");
	...
	public void unsubscribeTopic(Topic topic, User user) {
		...
		Session session = sessionFactory.openSession();
		session.beginTransaction();
		Query query = session.createQuery("delete from Subscription where user=? and topic=?");
		query.setEntity(0, user);
		query.setEntity(1, topic);
		query.executeUpdate();
	...
	public Long getNumberOfSubscriptionsOfTopic(Topic topic) {
		...
		Session session = sessionFactory.openSession();
		session.beginTransaction();
		Query query = session.createQuery("select count(*) from Subscription where topic=?");
